  ...
  try {
    ...
    const checklist = [
      ...
      },
      {
        ...
        description: "Your KYC must be approved by admin",
        progress: isKycApproved ? 100 : 0,
        current: isKycApproved ? 1 : 0,
        required: 1,
    ...
    res
      .status(500)
      .json({ success: false, message: "Failed to get KYC status" });
  }
};
  ...
  try {
    ...
    if (!ownershipProgress.isEligibleForKYC) {
      return res.status(400).json({
        success: false,
        message:
          "You are not eligible for KYC yet. Complete all requirements first.",
    ...
    const kyc = await KYC.create({
      ...
      document: {
        ...
        backImage: req.files.documentBack
...
const getKYCDetails = async (req, res) => {
  try {
    ...
    res
      .status(500)
      .json({ success: false, message: "Failed to get KYC details" });
  }
};
...
const resubmitKYC = async (req, res) => {
  try {
    ...
    if (documentNumber) {
      existingKYC.document.number = documentNumber;
    }
